...
class OnlineManager:
    ...
    def get_messages(self) -> list[dict]:
        """Legacy support for chat overlay"""
        with self._lock:
    ...
    def update(self, x: float, y: float, map_name: str, direction: str, moving: bool) -> bool:
        """Queue position update with full state."""
        if self.player_id == -1:
            return False
        try:
    ...
    async def _handle_message(self, message: str) -> None:
        ...
        try:
            ...
            if msg_type == "registered":
                ...
                Logger.info(f"Received players update: {list(players_data.keys())} | My ID: {self.player_id}")
                with self._lock:
                    filtered = []
                    for pid_str, p_data in players_data.items():
                        pid = int(pid_str)
                        if pid != self.player_id:
                            filtered.append({
                                ...
                                "map": str(p_data.get("map", "")),
                                "direction": str(p_data.get("direction", "down")),
                                "moving": bool(p_data.get("moving", False))
                            })
                ...
                with self._lock:
                    for m in messages:
                        ...
                        mid = int(m.get("idx", self._last_chat_id))
    ...
    async def _ws_sender(self, websocket: Any) -> None:
        ...
        while not self._stop_event.is_set():
            try:
                ...
                # Send chat messages
                try:
                    while not self._chat_out_queue.empty():
